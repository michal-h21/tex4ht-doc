...
Conversion is invoked using the \texttt{make4ht} command:
...
Let us start with conversion of simple \LaTeX\ file to HTML  
...
As you can see, multiple options can be joined for \texttt{make4ht}. The above invocation is equal to the following:
...
The option \texttt{--lua} tells \texttt{make4ht} to use Lua\LaTeX\ as a compilation engine. There is also a \texttt{-x} (or \texttt{--xetex}) option that allows the 
use of Xe\LaTeX\ for compilation. If neither of these options is used, the file
will be converted with the default PDF\LaTeX\ engine.


The \texttt{--mode} option sets 
...
\texttt{make4ht} converts \LaTeX\ file to a HTML 5 document. You can request
conversion to other formats using the \texttt{-f} option. For example,
to convert a document to the OpenDocument Format, use:

\begin{shellcommand}
make4ht -f odt filename.tex
\end{shellcommand}

More information about \texttt{make4ht} and it's command line options and other features can be found in the
...
Czech is the main document language, and English is secondary. 
Note the usage of the \texttt{otherlanguage} environment. 
It is provided by the \package{babel} package and locally
switches document languages, so correct hyphenation and other language-dependent 
stuff are used. We could use the
\texcommand{\selectlanguage} command, but I would like to
discourage the usage of switching commands such as this one, or font
switching commands like \texcommand{\bfseries}, for one
reason: it is impossible to configure them correctly for end element
insertion. 

For font-switch commands, situation is saved by the
...
But generally, such commands do not play nice with the nature of
\texttt{XML} based formats, where every started element must be closed
at the same hierarchical level. 
The use of \texttt{otherlanguage} environment will allow us to make the proper
