...
compilation engine. There is also a \texttt{-x} (or \texttt{--xetex}) option
...
For example, if we compiled the sample file without the \texttt{-l} option, we would get
a different result:

\texinput{examples/otherlang/babel-body.html}

Notice that the accents in \textit{ť} and \textit{ď} letters are detached from the base letters.
That's because \texfourht\ uses information about characters from the DVI file. Current \LaTeX
supports basic accented characters out of the box, but sometimes, they don't work as expected.


The \texttt{--mode} option sets the compilation mode. \makefourht\  has one built-in mode,
named \texttt{draft}. 
By default, \makefourht\  compiles your \TeX\ file three times, to get correct hyperlinks 
...
More information about \makefourht\ and it's command line options and other features can be found in the
...
If you have a big document, you may want to use separate page for each chapter, with a list of footnotes
at the end of these chapters. You can use following options: 
