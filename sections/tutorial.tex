...
As you can see, you can join multiple options for \texttt{make4ht}. The above invocation is equal to the following:
...
The \texttt{--lua} option tells \texttt{make4ht} to use Lua\LaTeX\ as an engine
for compilation. There is also \texttt{-x} (or \texttt{--xetex}) option that enables 
use of Xe\LaTeX\ for the compilation. If neither of these options is used, the file
...
It is provided by \package{babel} package and locally
