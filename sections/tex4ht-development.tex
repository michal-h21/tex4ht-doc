...
The usual \file{.4ht} files are loaded only after the \verb|\begin{document}|
...
cases, \TeX4ht provides a special configuration file \file{usepackage.4ht},
...
\begin{texsource}
% block unicode-math package
\:dontusepackage{unicode-math}
% provide dummy definition for \setmathfont command
...
\begin{itemize}
  \item \DocCommand{:dontusepackage}\marg{package name} – prevents the named
    package from loading.
  \item \DocCommand{:AtEndOfPackage}\marg{code} – executes the given code after
...
\file{usepackage.4ht}. For example, to disable footnote superscripts only in
...
is to use the file \file{usepackage-user.4ht}. This file works in the same way
as \file{usepackage.4ht}, but it is intended specifically for user-level
