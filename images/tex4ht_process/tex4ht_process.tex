...
\begin{tikzpicture}[node distance=20mm, empty/.style={inner sep=0,minimum size=0},file/.style={rectangle,dashed,draw=black}, command/.style={rectangle,minimum size=2em,very thick,draw=black},font=\ttfamily,draw=black]
  ...
  \node (split) [empty,right of=tex4ht] {};
  \node (split above) [empty, above of=split] {};
  \node (split below) [empty, below of=split] {};
  \node (xidv) [file,right of=split] {x.idv};
  \node (html) [file,above of=xidv] {html files};
  \node (xlg) [file,below of=xidv] {x.lg};
  \node (split right) [empty,right of=xidv] {};
  \node (split right down) [empty,right of=xlg] {};
  \node (join) [empty] at ($(split right)!0.5!(split right down)$) {};
  \node (t4ht) [command,right of=join] {t4ht};
  \node (pictures) [file,right of=t4ht,node distance=35mm] {images \& CSS files};
